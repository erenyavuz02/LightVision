...
class TextTransformer(nn.Module):
    def __init__(self, cfg: dict, projection_dim: int, *args, **kwargs) -> None:
        ...
        model_dim = cfg["dim"] # hidden_dim
        no_scale_embedding = cfg.get("no_scale_embedding", False) # use scale embedding or not
        no_pos_embedding = cfg.get("no_pos_embedding", False) # use positional embedding or not
        embed_dropout = cfg.get("embed_dropout", 0.0) # dropout for embedding
        norm_layer = cfg["norm_layer"] # normalization layer
        variant = cfg["model_name"] # model variant
        self.vocab_size = cfg["vocab_size"] # vocab size
        self.projection_dim = projection_dim # projection dimension which is used for final projection
        ...
        # Context length
        context_length = cfg["context_length"] # TODO: increase context length
        assert (
